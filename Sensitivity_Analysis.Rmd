...
clim=read_csv("climate_input.csv")%>%
  ...
  mutate(min_temp_coeff=rnorm(mean=-0.015, sd=sd1, n=1))%>%
  mutate(min_temp_coeff2=rnorm(mean=-0.0046, sd=sd2, n=1))%>%
  mutate(precip_coeff=rnorm(mean=-0.07, sd=sd3, n=1))%>%
  mutate(precip_coeff2=rnorm(mean=-0.004, sd=sd4, n=1))%>%
  mutate(intercept=rnorm(mean=0.28, sd=sd5, n=1))
...
tmp= mapply(FUN=Almond_Yield,min_temp_coeff=sens_pars$min_temp_coeff, min_temp_coeff2=sens_pars$min_temp_coeff2,precip_coeff=sens_pars$precip_coeff, precip_coeff2=sens_pars$precip_coeff2, intercept=sens_pars$intercept, min_temp = clim$min_temp, precip=clim$precip, mean_only=TRUE)
head(tmp)
```
...
sens_results = matrix((unlist(tmp)), ncol=1, byrow=TRUE)
...
ggplot(both1, aes(precip_coeff2, yield)) +
  geom_point() +
...
# Looking at the response to the two most important variables
ggplot(both2, aes(precip_coeff2, yield)) +
  geom_point() +
  labs(x = "Precipitation Coefficient 2", y = "Yield Anamoly")
